...
export default function LifeAnchorsLongevityView({ chartData, language }) {
  ...
  const translateOrganVulnerability = (text) => {
    if (!text || language !== 'ta') return text;
    if (text.includes('Mesha / Aries in 6th')) return 'தலைப்பகுதி, மூளை இரத்த ஓட்டம் மற்றும் அழற்சி தலைவலி உணர்திறன் (மேஷம் / 6-ஆம் பாவகம்)';
    if (text.includes('Vrishabha / Taurus in 6th')) return 'தொண்டை, குரல்வளை, தைராய்டு சுரப்பி மற்றும் முக திசு உணர்திறன் (ரிஷபம் / 6-ஆம் பாவகம்)';
    if (text.includes('Mithuna / Gemini in 6th')) return 'சுவாசக்குழாய், நரம்பு மண்டலம் மற்றும் தோள்பட்டை/கை நரம்பு இறுக்கம் (மிதுனம் / 6-ஆம் பாவகம்)';
    if (text.includes('Kataka / Cancer in 6th')) return 'நெஞ்சு/இரைப்பை செரிமானம், சளி சவ்வு மற்றும் உணர்ச்சி சார்ந்த மன அழுத்தம் (கடகம் / 6-ஆம் பாவகம்)';
    if (text.includes('Simha / Leo in 6th')) return 'மேல் வயிறு, ஜாடராக்னி (செரிமான தீ) மற்றும் இதய சுற்றோட்ட பலம் (சிம்மம் / 6-ஆம் பாவகம்)';
    if (text.includes('Kanya / Virgo in 6th')) return 'குடல் பகுதி, சத்து உறிஞ்சுதல் மற்றும் குடல் நுண்ணுயிரி சமநிலை (கன்னி / 6-ஆம் பாவகம்)';
    if (text.includes('Tula / Libra in 6th')) return 'சிறுநீரக மண்டலம், இடுப்பு முதுகுத்தண்டு மற்றும் நீர் வடிகட்டுதல் சமநிலை (துலாம் / 6-ஆம் பாவகம்)';
    if (text.includes('Vrishchika / Scorpio in 6th')) return 'இடுப்பு கூம்பு பகுதி, கழிவு வெளியேற்ற பாதைகள் மற்றும் இனப்பெருக்க திசு நலம் (விருச்சிகம் / 6-ஆம் பாவகம்)';
    if (text.includes('Dhanus / Sagittarius in 6th')) return 'கல்லீரல் வளர்சிதை மாற்றம், தமனி சுழற்சி மற்றும் தொடை/இடுப்பு தசை வலிமை (தனுசு / 6-ஆம் பாவகம்)';
    if (text.includes('Makara / Capricorn in 6th')) return 'முழங்கால் மூட்டுகள், எலும்பு அடர்த்தி மற்றும் மூட்டு திரவ ஒழுங்குமுறை (மகரம் / 6-ஆம் பாவகம்)';
    if (text.includes('Kumbha / Aquarius in 6th')) return 'கால் கணுக்கால், நரம்பு சுழற்சி மற்றும் புற இரத்த ஓட்ட நலம் (கும்பம் / 6-ஆம் பாவகம்)';
    if (text.includes('Meena / Pisces in 6th')) return 'நிணநீர் வடிகால், ஆழ்ந்த தூக்க சமநிலை மற்றும் பாத நரம்பு பலம் (மீனம் / 6-ஆம் பாவகம்)';

    if (text.includes('Sun as Roga Lord')) return 'இதய பலம், கண் பார்வை தெளிவு மற்றும் எலும்பு தாது உறிஞ்சுதல் (சூரியன் - ரோகாதிபதி)';
    if (text.includes('Moon as Roga Lord')) return 'உடல் திரவ சமநிலை, நிணநீர் ஒழுங்குமுறை மற்றும் மன அமைதி சமநிலை (சந்திரன் - ரோகாதிபதி)';
    if (text.includes('Mars as Roga Lord')) return 'இரத்த சுத்தி, தசை அழற்சி மற்றும் பித்த உஷ்ணம் தணித்தல் (செவ்வாய் - ரோகாதிபதி)';
    if (text.includes('Mercury as Roga Lord')) return 'தோல் பாதுகாப்பு அரண், நரம்பு மண்டலம் மற்றும் செரிமான என்சைம் சமநிலை (புதன் - ரோகாதிபதி)';
    if (text.includes('Jupiter as Roga Lord')) return 'கல்லீரல் கொழுப்பு வளர்சிதை மாற்றம் மற்றும் தமனி ஆரோக்கியம் (குரு - ரோகாதிபதி)';
    if (text.includes('Venus as Roga Lord')) return 'சிறுநீரக நீரேற்றம், நாளமில்லா சுரப்பி சமநிலை மற்றும் இனப்பெருக்க திசு ஆரோக்கியம் (சுக்கிரன் - ரோகாதிபதி)';
    if (text.includes('Saturn as Roga Lord')) return 'மூட்டு இயக்கம், வறட்சி தவிர்த்தல், தசைநார் நெகிழ்வுத்தன்மை (சனி - ரோகாதிபதி)';
  ...
  const translateLifestyleDirective = (text) => {
    if (!text || language !== 'ta') return text;
    if (text.includes('cooling, grounding, fresh whole foods')) return 'இயற்கையான இனிப்பு, கசப்பு மற்றும் துவர்ப்பு சுவை கொண்ட குளிர்ச்சியான, புத்துணர்ச்சியூட்டும் முழு உணவுகளை உட்கொள்ளவும்.';
    if (text.includes('Limit pungent spices, sour citrus excess')) return 'காரமான மசாலாக்கள், அதிக புளிப்பு/சிட்ரஸ், எண்ணெயில் பொரித்த உணவுகள் மற்றும் இரவு நேர கனமான உணவுகளைத் தவிர்க்கவும்.';
    if (text.includes('Favor warm, nourishing, easily digestible')) return 'மிதமான நெய், நல்லெண்ணெய் சேர்த்த சூடான, எளிதில் செரிமானமாகும் சத்தான சமைத்த உணவுகளை உட்கொள்ளவும்.';
    if (text.includes('Maintain consistent meal schedules')) return 'வழக்கமான நேரத்திற்கு உணவருந்தவும்; உலர்ந்த, குளிர்ந்த, பச்சையான மற்றும் கார்பனேற்றப்பட்ட உணவுகளைத் தவிர்க்கவும்.';
    if (text.includes('Favor light, warm, dry, and mildly spiced')) return 'செரிமான அக்னியைத் தூண்ட இலகுவான, சூடான மற்றும் மிதமான மசாலா சேர்த்த உணவுகளை உட்கொள்ளவும்.';
    if (text.includes('Minimize heavy dairy, refined sugars')) return 'கனமான பால் பொருட்கள், சர்க்கரை, குளிர்பானங்கள் மற்றும் உணவுக்குப் பின் உடனடியாக உறங்குவதைத் தவிர்க்கவும்.';
    if (text.includes('Incorporate gentle daily Pranayama')) return 'ஓஜஸ் (உயிர் நோய் எதிர்ப்பு சக்தி) பாதுகாக்க தினசரி நாடி சுத்தி/சீத்தளி பிராணாயாமம் மற்றும் சீரான தூக்க முறையைப் பின்பற்றவும்.';
    return text;
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '22px' }}>
      ...
      <div className="card" style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.03) 0%, rgba(212,175,55,0.06) 100%)', border: '1px solid rgba(212,175,55,0.25)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px', marginBottom: '16px' }}>
          <h3 style={{ margin: 0, color: 'var(--accent-gold)', display: 'flex', alignItems: 'center', gap: '8px' }}>
            🕉️ {language === 'ta' ? 'தெய்வங்கள் & ஆன்மீக நங்கூரங்கள் (Spiritual Anchors)' : 'Spiritual & Deity Anchors'}
          </h3>
          <span style={{ fontSize: '12px', padding: '4px 12px', borderRadius: '14px', background: 'rgba(212,175,55,0.15)', color: 'var(--accent-gold)', border: '1px solid rgba(212,175,55,0.3)', fontWeight: 'bold' }}>
            {deities?.karakamsaSignD9 ? `${language === 'ta' ? 'காரகாம்சம்' : 'Karakamsa'}: ${deities.karakamsaSignD9} (D9)` : 'Karakamsa Anchor'}
          </span>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '14px', marginBottom: '16px' }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)', borderRadius: '8px', padding: '14px' }}>
            <div style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '4px' }}>
              🙏 {language === 'ta' ? 'இஷ்ட தெய்வம் (Ishta Devata - 12th from Karakamsa)' : 'Ishta Devata (12th from Karakamsa)'}
            </div>
          ...
          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)', borderRadius: '8px', padding: '14px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
              <span style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>
                🪔 {language === 'ta' ? 'குலதெய்வ அருள் நிலை (Kula Devata Status)' : 'Kula Devata Status (5th House)'}
              </span>
              <span style={{
                fontSize: '11px',
                fontWeight: 'bold',
                ...
                {deities?.kulaDevataBlessingStatus === 'BLESSED' ? (language === 'ta' ? 'பரிபூரண ஆசி' : 'Blessed') : (language === 'ta' ? 'பிரார்த்தனை தேவை' : 'Remedy Advised')}
              </span>
            </div>
          ...
          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)', borderRadius: '8px', padding: '14px' }}>
            <div style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '4px' }}>
              ⚖️ {language === 'ta' ? 'தர்ம தெய்வம் (Dharma Devata - 9th from Karakamsa)' : 'Dharma Devata (9th from Karakamsa)'}
            </div>
      ...
      <div className="card">
        <h3 style={{ margin: '0 0 16px 0', color: 'var(--accent-gold)', display: 'flex', alignItems: 'center', gap: '8px' }}>
          💎 {language === 'ta' ? 'அதிர்ஷ்ட ரத்தினம் & சாஸ்திர விதிகள் (Vedic Gemology)' : 'Vedic Gemology & Auspicious Gemstones'}
        </h3>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '14px', marginBottom: '16px' }}>
          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)', borderRadius: '8px', padding: '14px' }}>
            <div style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '4px' }}>
              👑 {language === 'ta' ? 'முதன்மை யோக ரத்தினம் (Primary Gem)' : 'Primary Auspicious Gem'}
            </div>
            <div style={{ fontSize: '18px', fontWeight: 'bold', color: 'var(--accent-gold)', marginBottom: '4px' }}>
              {gemology?.primaryGemstoneTamil || gemology?.primaryGemstone}
            </div>
            <div style={{ fontSize: '12px', color: '#3498db' }}>
              {language === 'ta' ? 'உபரத்தினம்' : 'Substitute'}: {gemology?.secondarySubstitute}
            </div>
          </div>

          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)', borderRadius: '8px', padding: '14px' }}>
            <div style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '4px' }}>
              💍 {language === 'ta' ? 'உலோகம் & விரல் (Metal & Finger)' : 'Recommended Metal & Finger'}
            </div>
          ...
          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)', borderRadius: '8px', padding: '14px' }}>
            <div style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '4px' }}>
              ⏰ {language === 'ta' ? 'அணியும் காலம் & முகூர்த்தம்' : 'Activation Timing & Day'}
            </div>
        ...
        {gemology?.forbiddenCompanionGems && gemology.forbiddenCompanionGems.length > 0 && (
          <div style={{ background: 'rgba(231, 76, 60, 0.08)', border: '1px solid rgba(231, 76, 60, 0.25)', borderRadius: '8px', padding: '12px' }}>
            <strong style={{ fontSize: '12px', color: '#e74c3c', display: 'block', marginBottom: '6px' }}>
              ⚠️ {language === 'ta' ? 'சேர்த்து அணியக் கூடாத பகை ரத்தினங்கள் (Incompatible Gemstone Warnings):' : 'Incompatible Companion Gemstones:'}
            </strong>
      ...
      <div className="card">
        <h3 style={{ margin: '0 0 16px 0', color: 'var(--accent-gold)', display: 'flex', alignItems: 'center', gap: '8px' }}>
          🔢 {language === 'ta' ? 'எண்கணிதம் & அதிர்ஷ்ட கூறுகள் (Numerology & Lucky Elements)' : 'Numerology & Lucky Elements'}
        </h3>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px', marginBottom: '18px' }}>
          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)', borderRadius: '8px', padding: '14px', textAlign: 'center' }}>
            <div style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '4px' }}>
              🚗 {language === 'ta' ? 'ஓட்டுநர் எண் (Driver No - DD)' : 'Radical / Driver Number'}
            </div>
            <div style={{ fontSize: '28px', fontWeight: 'bold', color: 'var(--accent-gold)' }}>
              {num?.radicalDriverNumber}
            </div>
            <div style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>
              {language === 'ta' ? 'அதிபதி' : 'Ruler'}: {num?.radicalRulingPlanet}
            </div>
          </div>

          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)', borderRadius: '8px', padding: '14px', textAlign: 'center' }}>
            <div style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '4px' }}>
              🎯 {language === 'ta' ? 'நடத்துனர் எண் (Conductor No - Full DOB)' : 'Destiny / Conductor Number'}
            </div>
            <div style={{ fontSize: '28px', fontWeight: 'bold', color: '#3498db' }}>
              {num?.destinyConductorNumber}
            </div>
            <div style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>
              {language === 'ta' ? 'அதிபதி' : 'Ruler'}: {num?.destinyRulingPlanet}
            </div>
          </div>

          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)', borderRadius: '8px', padding: '14px', textAlign: 'center' }}>
            <div style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '4px' }}>
              🪐 {language === 'ta' ? 'ஜோதிட லக்ன எண் (Planet Number)' : 'Astrological Planet Number'}
            </div>
            <div style={{ fontSize: '28px', fontWeight: 'bold', color: '#2ecc71' }}>
              {num?.astrologicalPlanetNumber}
            </div>
            <div style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>
              {language === 'ta' ? 'அதிபதி' : 'Ruler'}: {num?.astrologicalPlanetName}
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '12px', marginBottom: '18px' }}>
          <div style={{ background: 'rgba(46, 204, 113, 0.05)', border: '1px solid rgba(46, 204, 113, 0.25)', borderRadius: '8px', padding: '12px' }}>
            <div style={{ fontSize: '11px', color: '#2ecc71', fontWeight: 'bold', marginBottom: '6px' }}>
              💚 {language === 'ta' ? 'நட்பு எண்கள் (Friendly Numbers)' : 'Friendly Numbers'}
            </div>
          ...
          <div style={{ background: 'rgba(241, 196, 15, 0.05)', border: '1px solid rgba(241, 196, 15, 0.25)', borderRadius: '8px', padding: '12px' }}>
            <div style={{ fontSize: '11px', color: '#f1c40f', fontWeight: 'bold', marginBottom: '6px' }}>
              💛 {language === 'ta' ? 'சம எண்கள் (Neutral Numbers)' : 'Neutral Numbers'}
            </div>
            <div style={{ fontSize: '14px', fontWeight: 'bold', color: 'var(--text-primary)' }}>
          ...
          <div style={{ background: 'rgba(231, 76, 60, 0.05)', border: '1px solid rgba(231, 76, 60, 0.25)', borderRadius: '8px', padding: '12px' }}>
            <div style={{ fontSize: '11px', color: '#e74c3c', fontWeight: 'bold', marginBottom: '6px' }}>
              💔 {language === 'ta' ? 'பகை எண்கள் (Enemy Numbers to Avoid)' : 'Enemy Numbers to Avoid'}
            </div>
            <div style={{ fontSize: '14px', fontWeight: 'bold', color: 'var(--text-primary)' }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)', borderRadius: '8px', padding: '14px', marginBottom: '16px' }}>
          <h4 style={{ margin: '0 0 10px 0', fontSize: '13px', color: 'var(--accent-gold)' }}>
            📅 {language === 'ta' ? 'மாதாந்திர அதிர்ஷ்ட தேதிகள் (Monthly Lucky Dates Matrix)' : 'Monthly Lucky Dates Matrix'}
          </h4>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '10px' }}>
            <div>
              <span style={{ fontSize: '11px', color: '#2ecc71', display: 'block', marginBottom: '4px', fontWeight: 'bold' }}>
                🌟 {language === 'ta' ? 'முதன்மை அதிர்ஷ்ட தேதிகள்:' : 'Primary Lucky Dates:'}
              </span>
            ...
            <div>
              <span style={{ fontSize: '11px', color: '#3498db', display: 'block', marginBottom: '4px', fontWeight: 'bold' }}>
                ✨ {language === 'ta' ? 'துணை அதிர்ஷ்ட தேதிகள்:' : 'Secondary Friendly Dates:'}
              </span>
            ...
            <div>
              <span style={{ fontSize: '11px', color: '#e74c3c', display: 'block', marginBottom: '4px', fontWeight: 'bold' }}>
                ⛔ {language === 'ta' ? 'தவிர்க்க வேண்டிய தேதிகள்:' : 'Dates to Avoid:'}
              </span>
              <span style={{ fontSize: '13px', color: 'var(--text-primary)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '14px' }}>
          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)', borderRadius: '8px', padding: '14px' }}>
            <div style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '4px' }}>
              ☀️ {language === 'ta' ? 'அதிர்ஷ்ட கிழமை (Vedic Lucky Weekday - Vara)' : 'Vedic Lucky Weekday (Vara)'}
            </div>
          ...
          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)', borderRadius: '8px', padding: '14px' }}>
            <div style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '4px' }}>
              🧭 {language === 'ta' ? 'சுப திசைகள் (Auspicious Directions & Vastu)' : 'Auspicious Directions (Vastu & Travel)'}
            </div>
            <div style={{ fontSize: '13px', color: 'var(--text-primary)', marginBottom: '4px' }}>
              🏡 <strong>{language === 'ta' ? 'நிரந்தர வாஸ்து திசை' : 'Permanent Vastu'}:</strong> {dir?.permanentVastuDirection}
            </div>
            <div style={{ fontSize: '13px', color: 'var(--text-primary)' }}>
              ✈️ <strong>{language === 'ta' ? 'சுப பயண திசை' : 'Travel Direction'}:</strong> {dir?.travelDirection}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        <h3 style={{ margin: '0 0 16px 0', color: 'var(--accent-gold)', display: 'flex', alignItems: 'center', gap: '8px' }}>
          🏛️ {language === 'ta' ? 'கட்டமைப்பு நங்கூரங்கள் (Structural Astrological Anchors)' : 'Structural Astrological Anchors'}
        </h3>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '14px' }}>
          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)', borderRadius: '8px', padding: '14px' }}>
            <div style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '4px' }}>
              💪 {language === 'ta' ? 'உடல் & சரீர பல நங்கூரம் (Physical Vitality Anchor)' : 'Physical & Vitality Anchor'}
            </div>
            <div style={{ fontSize: '12px', color: 'var(--text-primary)', lineHeight: '1.5' }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)', borderRadius: '8px', padding: '14px' }}>
            <div style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '4px' }}>
              🌟 {language === 'ta' ? 'சமூக அந்தஸ்து & தொழில் நங்கூரம் (Arudha Lagna - AL)' : 'Social Status & Arudha Lagna (AL)'}
            </div>
            <div style={{ fontSize: '12px', color: 'var(--text-primary)', lineHeight: '1.5' }}>
              {struct?.arudhaLagna}
            </div>
          ...
          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)', borderRadius: '8px', padding: '14px' }}>
            <div style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '4px' }}>
              🧠 {language === 'ta' ? 'மன உறுதி & உணர்ச்சி நங்கூரம் (Mind Resilience Dispositor)' : 'Emotional & Mind Resilience Anchor'}
            </div>
            <div style={{ fontSize: '12px', color: 'var(--text-primary)', lineHeight: '1.5' }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)', borderRadius: '8px', padding: '14px' }}>
            <div style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '4px' }}>
              💰 {language === 'ta' ? 'யோக & கர்ம நங்கூரம் (SAV Karma Anchor)' : 'Prosperity & Karma Anchor (SAV)'}
            </div>
            <div style={{ fontSize: '12px', color: 'var(--text-primary)', lineHeight: '1.5' }}>
